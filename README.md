### DetectAI: Professional Text Verifier
DetectAI is a sophisticated tool designed to distinguish between human-written and AI-generated content. By combining traditional Machine Learning with modern Deep Learning architectures, it provides a dual-layer analysis for maximum accuracy.

✨ Key Features
Hybrid Architecture: Dual-engine analysis using XGBoost (ML) and TensorFlow (DL).

Glassmorphism UI: A sleek, non-scrollable professional interface built with Streamlit and custom CSS.

Real-time Metrics: Displays confidence scores, processing speed (~0.4s), and system architecture.

Responsive Design: Optimized for desktop viewing with dynamic color feedback (Green for Human, Red for AI).

### 🚀 Installation & Setup
### Clone the Repo:
```
bash

git clone https://github.com/YOUR_USERNAME/hybrid-text-detection.git
```

### Install Dependencies:


```
bash
pip install -r requirements.txt
```

### Run the App:
```
bash
streamlit run app.py
```
### 📁 Folder Structure


<li>app.py: Main Streamlit application.</li>
<li>models/: Pre-trained model files (.pkl and .keras).</li>
<li>requirements.txt: Python dependencies.</li>

